...
- (void)drawView
{
    ...
    if(length >= 0.1)
	{
		...
		centerImage.center = CGPointMake(centerImage.center.x+(xVal-oldx)*40, centerImage.center.y+ (-(yVal-oldy))*10);
        }
        else if((xVal-oldx) < -0.01 && (yVal-oldy) >0.01){
            centerImage.center = CGPointMake(centerImage.center.x+(xVal-oldx)*10, centerImage.center.y+ (-(yVal-oldy))*10);
        }
        else if((xVal - oldx) < -0.01 && (yVal -oldy) < -0.01){
            centerImage.center = CGPointMake(centerImage.center.x+(xVal-oldx)*10, centerImage.center.y+ (-(yVal-oldy))*10);
        }
